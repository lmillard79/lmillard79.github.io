...
The downscaling paper (the one I opened with) is their second example, and it's making the same point from a different angle: which model "wins" depends entirely on what you're asking it to be good at. Presented as a bare accuracy claim, that's a number that invites over-trust. Presented with the actual benchmark broken out by metric and region, it's something a practitioner can actually use to pick the right tool for their specific problem.
